...
if ! ${USE_MANUAL_LABELS}; then
  ...
  echo -e "\tDRPNet ... "
  export REPIC_OOB=1
  export REPIC_OUT_DIR=${SUB_DIR}/${LABEL}/drpnet/BOX
  export REPIC_MRC_DIR=${IN_DIR}/data
  bash "${REPIC}/iterative_particle_picking/run_drpnet.sh"
  #bash ${REPIC}/iterative_particle_picking/run_drpnet.sh &> ${REPIC_OUT_DIR}/run_drpnet.log
  ...
  # train consensus
  mkdir -p ${TMP}/{crYOLO,deepPicker,topaz,drpnet}
  ...
  cp -s ${SUB_DIR}/${LABEL}/drpnet/BOX/train/*.box ${TMP}/drpnet/
  ...
  cp -s ${SUB_DIR}/${LABEL}/drpnet/BOX/val/*.box ${TMP}/drpnet/
  ...
  cp -s ${SUB_DIR}/${LABEL}/drpnet/BOX/test/*.box ${TMP}/drpnet/
...
while [ ${i} -lt ${NUM_ROUNDS} ]; do
  ...
  echo -e "\tDRPNet ...  "
  #drpnet
  export REPIC_OUT_DIR=${SUB_DIR}/${LABEL}/drpnet
  export REPIC_MRC_DIR=${IN_DIR}/data
  bash "${REPIC}/iterative_particle_picking/fit_drpnet.sh"
  #bash ${REPIC}/iterative_particle_picking/fit_drpnet.sh &> ${REPIC_OUT_DIR}/fit_drpnet.log
  ...
  cp -fs ${SUB_DIR}/${LABEL}/drpnet/BOX/train/*.box ${TMP}/drpnet/
